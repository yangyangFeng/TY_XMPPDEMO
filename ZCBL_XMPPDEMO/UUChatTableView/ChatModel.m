...
//    [dataDic setObject:@(UUMessageFromMe) forKey:@"from"];
    [dataDic setObject:[[NSDate date] description] forKey:@"userTime"];
//    [dataDic setObject:@"Hello,Sister" forKey:@"userName"];
    [dataDic setObject:URLStr forKey:@"userIcon"];
    if ([[dic objectForKey:@"from"] integerValue]== UUMessageFromMe) {
        NSString * urlIcon = @"http://img0.bdstatic.com/img/image/shouye/xinshouye/mingxing15.jpg";
        [dataDic setObject:urlIcon forKey:@"userIcon"];
    }
    [message setWithDict:dataDic];
    [message minuteOffSetStart:previousTime end:dataDic[@"userTime"]];
    messageFrame.showTime = message.showDateLabel;
    [messageFrame setMessage:message];
    
    if (message.showDateLabel) {
        previousTime = dataDic[@"userTime"];
    }
...
- (NSDictionary *)getDic
{
    ...
    [dictionary setObject:[date description] forKey:@"userTime"];
    // 这里判断是否是私人会话、群会话
    int index = _isGroupChat ? arc4random()%6 : 0;
    [dictionary setObject:[self getName:index] forKey:@"userName"];
    [dictionary setObject:[self getImageStr:index] forKey:@"userIcon"];
